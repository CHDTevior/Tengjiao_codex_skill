...
6. Any shell step that invokes `codex exec` must include `{CODEX_FULL_ACCESS_FLAG}` by default,
   especially in `.codex-research/run_one_task.sh` and `.codex-research/run_plan.sh`.
...
def _enforce_shell_codex_full_access(content: str) -> str:
    if "codex" not in content or "exec" not in content:
        return content
    ...
    for idx, line in enumerate(lines):
        if not re.search(r"\bcodex\s+exec\b", line):
            continue
        if CODEX_FULL_ACCESS_FLAG in line:
            continue
        lines[idx] = re.sub(
            r"\bcodex\s+exec\b",
            f"codex exec {CODEX_FULL_ACCESS_FLAG}",
            line,
            count=1,
        )
        changed = True
    ...
    for idx in range(len(lines) - 1):
        if CODEX_FULL_ACCESS_FLAG in lines[idx] or CODEX_FULL_ACCESS_FLAG in lines[idx + 1]:
            continue
        if re.search(r"\bcodex\b", lines[idx]) and re.search(r"^\s*exec\b", lines[idx + 1]):
            lines[idx + 1] = re.sub(
                r"^(\s*)exec\b",
                rf"\1exec {CODEX_FULL_ACCESS_FLAG}",
                lines[idx + 1],
                count=1,
            )
            changed = True
...
def _materialize_generated_bundle(
    *,
    ...
    codex_targets: List[str] = [
        ...
        if p not in META_JSON_PATHS and p != CN_EXEC_GUIDE_PATH
    ]
    ...
    for rel_path in codex_targets:
        ...
        if rel_path.endswith((".sh", ".sbatch")):
            content = _enforce_shell_codex_full_access(content)
