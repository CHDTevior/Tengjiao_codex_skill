...
- 不再用关键词或任务行的规则提取来决定文件内容。
- 现在由 Codex 在一次调用中返回完整文件包（含元数据和按顺序的文件内容），然后本地脚本做结构校验与落盘。
...
- `--codex-plan-stage required|auto|off`
  ...
  - `auto`：尝试标准化，失败时回退原计划
  - `off`：跳过标准化
- `--codex-model`：指定 Codex 模型，会同时传给对齐和生成两个阶段
...
1. 参数和路径校验  
   检查 `--plan`、`--target` 和相关脚本是否可用。

2. 计划标准化（可选）  
   调用 `scripts/normalize_plan_with_codex.sh`，让 Codex 把原始计划整理成结构化 `normalized_plan.md`，并做 section / 格式校验。

3. Codex 生成文件包（核心）  
   调用 `scripts/codex_research_harness.py`。在 `bootstrap` 或 `all` 模式下，该脚本使用一次 `codex exec` 配合 JSON schema，让 Codex 直接返回：
   - `required_files`
   - `feature_list`
   - `task_plan`
   - `files`（按 `required_files` 顺序返回的完整文件包）

   本地脚本会校验：
   - 必需核心文件是否齐全
   - 所有路径是否位于 `.codex-research/`
   - `feature_list` / `task_plan` 结构是否合法，且默认 `passes=false`
   - `files` 与 `required_files` 的路径集合和顺序是否一致
   - 不改写生成脚本去强行注入执行 bypass 参数
   - 若存在 `.codex-research/workflow/CODEX.md`，自动补齐 GitHub 维护渠道章节（若已存在则不重复添加）

4. 写入目标目录  
   将生成内容写入 `<target>/.codex-research/`，并为 shell 文件设置可执行位。

5. 给出执行下一步（中文指南）  
   不再生成 `run_one_task.sh` 或 `run_plan.sh`。生成结束后，统一通过 `.codex-research/execution_guide.zh-CN.md` 指导后续使用方式，包括：
